<template>
    <div class="max-w-xl">
        <div class="card">
            ...
            <p v-if="error" v-text="error" class="my-2 text-red-500 text-sm"></p>
        </div>
        <div v-if="loading" class="w-full flex flex-col items-center justify-center mt-4">
        ...
        <div v-if="success" class="w-full flex flex-col items-center justify-center mt-4">
...
<script>
    export default {
        ...
        methods: {
            ...
            fetchReport() {
                Statamic.$request.post(this.create_report_url, { domain: this.domain })
                    .then(({ data }) => {
                        if (! data.success) {
                            return;
                        }
                        ...
                        setTimeout(() => {
                            window.location.href = cp_url(`seo-pulse/${data.data.id}`) ;
                        }, 2000);
                    });
            }
        }
